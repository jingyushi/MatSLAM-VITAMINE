...
match_pairs = zeros(size(small,1),4);
min_distance = zeros(size(small,1),1);
for i = 1:size(small,1)
    for j = 1:size(large,1)
        distance(i,j) = hamming_distance(small(i,:),large(j,:));
    end
    [min_distance(i),index] = min(distance(i,:));
    match_pairs(i,:) = [f_small(i,1) f_small(i,2) f_large(index,1) f_large(index,2)];
