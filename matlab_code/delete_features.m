...
for i = 1:length(features_info)
    if (( features_info(i).times_measured < 0.8*features_info(i).times_predicted ) &&...
            ( features_info(i).times_predicted > 2 ))||(features_info(i).individually_compatible~=1)...
            ||((features_info(i).low_innovation_inlier==0)&&(features_info(i).high_innovation_inlier==0))
        deletion_list = [deletion_list; i];
    end
end
if(length(features_info)~=0)
    R = randperm(length(features_info),floor(length(features_info)/2));
    deletion_list = [deletion_list; R'];
    deletion_list = unique(sort(deletion_list));
    %length(features_info);
end
...
%         if(deletion_list(i)>length(features_info))
%             break;
%         end
