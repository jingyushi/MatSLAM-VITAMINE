...
function features_info = anj_matching_1( im, features_info, cam )
...
for i_feature=1:length(features_info) % for every feature in the map
    ...
    if ~isempty(features_info(i_feature).h); % if it is predicted, search in the region
        ...
        if eig(S)< 100 %if the ellipse is too big, do not search (something may be wrong)
            ...
            features = zeros((2*half_search_region_size_x+1)*(2*half_search_region_size_y+1) + 1 , 64);
                ...
                for i = round(h(2))-half_search_region_size_y:...
                    ...
                    if ( (nu'*invS*nu) < chi_095_2 )
                        if (j>half_patch_size_when_matching)&&(j<cam.nCols-half_patch_size_when_matching)&&...
                            ...
                            [a,b] = extractFeatures(im,points,'Method','SURF');
                            ...
                            %features_for_matching(index_patches_for_correlation,:) = obj.
                            match_candidates( :, index_patches_for_correlation - 1 ) = [ j; i ];
                            features(index_patches_for_correlation - 1, :) = a;
            ...
            indexPairs = matchFeatures(double(features_info(i_feature).feature),features, 'MatchThreshold', 60);
            
            if( ~isempty(indexPairs))
                features_info(i_feature).individually_compatible = 1;
                features_info(i_feature).z = match_candidates( :, indexPairs(1,2) );
            end
